...
class Player:
    ...
    def oldCompleteds(self):
        with requests.Session() as req:
            req.cookies['.ROBLOSECURITY'] = self.cookie
            oldAccepts = req.get('https://trades.roblox.com/v1/trades/completed?cursor=&limit=25&sortOrder=Desc').json()
    ...
    def newCompleteds(self):
        while True:
            ...
            with requests.Session() as req:
                req.cookies['.ROBLOSECURITY'] = self.cookie
                try:
                    newAccepts = req.get('https://trades.roblox.com/v1/trades/completed?cursor=&limit=25&sortOrder=Desc').json()
    ...
    def checkValuation(self, toSend):
        with requests.Session() as req:
            ...
            for tradeId in toSend:
                try:
                    r = req.get(f'https://trades.roblox.com/v1/trades/{tradeId}').json()
